...
class WorkoutsStore extends ChangeNotifier {
  ...
  Future<void> loadPlans() async {
    ...
    final exRows = await _sb
        .from('exercises')
        .select('id, plan_id, name, sets, reps, weight_kg, weights_kg, rest_seconds, created_at')
        .eq('user_id', userId)
  ...
  Future<WorkoutExercise> createExercise({
    ...
    final inserted = await _sb
        ...
        .insert({
          ...
          'weights_kg': (draft.weightsKg == null || draft.weightsKg!.isEmpty)
              ? null
              : draft.weightsKg,
          'rest_seconds': draft.restSeconds,
        })
       .select('id, name, sets, reps, weight_kg, weights_kg, rest_seconds')
        .single();
    ...
    final ex = WorkoutExercise(
      ...
      weightsKg: inserted['weights_kg'] == null
          ? null
          : List<double>.from(
              (inserted['weights_kg'] as List).map(
                (e) => (e as num).toDouble(),
              ),
            ),
